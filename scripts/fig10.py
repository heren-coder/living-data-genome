...
a1=fig.add_axes([0.115,0.215,0.205,0.520])
a2=fig.add_axes([0.400,0.215,0.205,0.520])
a3=fig.add_axes([0.735,0.215,0.245,0.520])
...
for i in range(3):
    for j in range(3):
        ...
        a1.text(j,i+0.10,f"{v:.2f}",ha="center",va="center",fontsize=8.0,
                color=(GRN if v>0.6 else BK),fontweight="bold")
        a1.text(j,i-0.22,f"{Mh[i,j]:.2f}",ha="center",va="center",fontsize=6.6,color=ACC)
...
a1.set_title("pairwise agreement",fontsize=7.6,
             color="#0F2545",fontweight="bold",pad=18)
fig.text(0.2175,0.055,"RLV in bold, healthcare below; chance is 0.25",ha="center",
         fontsize=6.4,color=BK,style="italic")
...
for yi,r in zip(y,rows):
    a2.text(0.03,yi+0.40,r[1],fontsize=6.2,color=BK,style="italic",va="center")
a2.set_xlim(0,1.18); a2.set_ylim(1.75,-0.62)
a2.set_xlabel("consistency at its own level",fontsize=7.2,color=BK)
...
a2.legend(fontsize=6.8,frameon=False,loc="upper center",bbox_to_anchor=(0.5,1.15),ncol=2,
...
a3.annotate("two coordinates equally tight",xy=(w.gap,w.pool_conc),xytext=(0.006,0.52),
            fontsize=6.2,color=BK,arrowprops=dict(arrowstyle="-|>",color=BK,lw=0.8,mutation_scale=6))
a3.set_xlabel("margin between the two tightest",fontsize=7.2,color=BK)
...
a3.legend(fontsize=6.8,frameon=False,loc="lower right",labelcolor=BK)
a3.set_title("Corollary C2: geometry fixes it",fontsize=7.6,color="#0F2545",fontweight="bold",pad=18)
